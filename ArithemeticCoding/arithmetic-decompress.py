import arithmeticcoding
import hashlib
import os
import time
from datetime import timedelta

# Command line main application function.
def main(inputfile, outputfile):
	# Handle command line arguments
	# if len(args) != 2:
	# 	sys.exit("Usage: python arithmetic-decompress.py InputFile OutputFile")
	# inputfile, outputfile = args
	
	# Perform file decompression
	with open(outputfile, "wb") as out, open(inputfile, "rb") as inp:
		bitin = arithmeticcoding.BitInputStream(inp)
		freqs = read_frequencies(bitin)
		decompress(freqs, bitin, out)
	print('Decompress Success !!!')

def read_frequencies(bitin):
	def read_int(n):
		result = 0
		for _ in range(n):
			result = (result << 1) | bitin.read_no_eof()  # Big endian
		return result
	
	freqs = [read_int(32) for _ in range(256)]
	freqs.append(1)  # EOF symbol
	return arithmeticcoding.SimpleFrequencyTable(freqs)


def decompress(freqs, bitin, out):
	dec = arithmeticcoding.ArithmeticDecoder(32, bitin)
	while True:
		symbol = dec.read(freqs)
		if symbol == 256:  # EOF symbol
			break
		out.write(bytes((symbol,)))


# Main launcher
if __name__ == "__main__":

	absolute_path = os.path.dirname(os.path.abspath("main.py"))

	compressed_file_folder = absolute_path + "/CompressedFiles/"
	decompressed_file_folder = absolute_path + "/DecompressedFiles/"
	original_file_folder = absolute_path + "/OriginalFiles/"

	input_file_name = input("Enter your filename: ")
	input_file = compressed_file_folder + input_file_name

	print("Input filename: " + input_file)

	# Get size of file input
	file_stats_input = os.stat(input_file)
	print(f'Input filename Size in Bytes is {file_stats_input.st_size}')

	output_file = decompressed_file_folder + input_file_name + "_decompressed"
	if not os.path.exists(output_file):
		open(output_file, 'w')

	# Get MD5 of decompressed file
	filehash_output = hashlib.md5()
	filehash_output.update(open(output_file, 'rb').read())
	print("decompressed_file: " + output_file + " : "  + filehash_output.hexdigest())

	start_time = time.time()

	main(input_file, output_file)

	elapsed_time_secs = time.time() - start_time

	# Get size of file output
	file_stats_output = os.stat(output_file)
	print(f'Output filename Size in Bytes is {file_stats_output.st_size}')

	# Get MD5 of original file
	original_file = original_file_folder + input_file_name[:len(input_file_name)-11:]
	filehash_origin = hashlib.md5()
	filehash_origin.update(open(original_file, 'rb').read())
	print("origin_file: " + original_file + " : " + filehash_origin.hexdigest())

	# Check MD5
	if(filehash_output.hexdigest() == filehash_origin.hexdigest()):
		print("OK")

	# Get time execution
	msg = "Execution took: %s secs (Wall clock time)" % timedelta(seconds=round(elapsed_time_secs))
	print(msg)