...
class ArithmeticEncoder(ArithmeticCoderBase):
	...
	# Constructs an arithmetic coding encoder based on the given bit compress_file stream.
	def __init__(self, numbits, bitout):
		super(ArithmeticEncoder, self).__init__(numbits)
		# The underlying bit compress_file stream.
		self.output = bitout
	...
	# Terminates the arithmetic coding by flushing any buffered bits, so that the compress_file can be decoded properly.
	# It is important that this method must be called at the end of the each encoding process.
	# Note that this method merely writes data to the underlying compress_file stream but does not close it.
	def finish(self):
		self.output.write(1)
...
class BitOutputStream:
	...
	# Constructs a bit compress_file stream based on the given byte compress_file stream.
	...
	# Closes this stream and the underlying compress_file stream. If called when this
